...
class SearchSpace(object):
    def __init__(self, dimension_lengths, O=None):
        ...
        if O is None:
            ...
            # self.obs_centers = [tuple(((obs[0]+obs[2])/2, (obs[1]+obs[3])/2)) for obs in O]
            self.obs_centers = [self.obs_center(obs) for obs in O]

    def obs_center(self, obs):
        center = tuple()
        for i in range(self.dimensions):
            center += ((obs[i] + obs[i + self.dimensions]) / 2,)
        return center
    ...
    @staticmethod
    def judge(M, N, P):
        return (P[1]-M[1])*(N[0]-M[0]) > (N[1]-M[1])*(P[0]-M[0])
